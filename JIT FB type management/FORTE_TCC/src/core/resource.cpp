  ...
  if(CStringDictionary::scm_nInvalidStringId == paCommand.mDestination){
    switch (paCommand.mCMD){
      case cg_nMGM_CMD_Create_FBInstance: {
		...
		if (retVal == e_UNSUPPORTED_TYPE)
		{
			//regisiter new type based on *.fbt files
			CFunctionBlock* newFB;
			newFB = handleUnknownType(paCommand.mFirstParam.front(), paCommand.mSecondParam.front(), true);
			if (0 != newFB)
			{
