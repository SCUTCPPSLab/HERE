...
void CCompositeFB::createInternalFBs(){
  if(cm_cpoFBNData->m_nNumFBs){
    ...
    for(unsigned int i = 0; i < cm_cpoFBNData->m_nNumFBs; ++i){
	  ...
	  if (m_apoInternalFBs[i] ==NULL)
	  {
		  m_apoInternalFBs[i] = this->getResourcePtr()->handleUnknownType(cm_cpoFBNData->m_pstFBInstances[i].m_nFBInstanceNameId, cm_cpoFBNData->m_pstFBInstances[i].m_nFBTypeNameId, true);
	  }
#ifdef FORTE_SUPPORT_MONITORING
